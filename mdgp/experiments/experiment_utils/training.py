...
class TrainingArguments: 
    ...
    sample_hidden: str = field(
        default='elementwise', 
        metadata={'help': 'Name of the function to sample from the hidden space. Must be one of ["elementwise", "pathwise"]'})


def train_step(model, inputs, targets, criterion, sample_hidden='elementwise', loggers=None, step=None): 
    model.train() 
...
def test_step(model, inputs, targets, sample_hidden='elementwise', train_targets=None, loggers=None, step=None):
    with no_grad():
        model.eval() 
