...
class PosteriorSampler(torch.nn.Module): 
    ...
    def sample_prior(self, x, z, sample_shape: torch.Size = torch.Size([]), resample=True):
        Phi_w_x = self.rff_sampler(x, sample_shape=sample_shape, resample=resample)
        Phi_w_z = self.rff_sampler(z, sample_shape=sample_shape, resample=False)
    ...
    def compute_posterior_update(self, x, z, u, Phi_w_z, sample_shape: torch.Size = torch.Size([])): 
        k_x_z = self.rff_sampler.covar_module(x, z) # [O, N, M] or [N, M]
        K_z_z = self.rff_sampler.covar_module(z, z) # [O, M, M] or [M, M]
    ...
    def forward(self, x, sample_shape: torch.Size = torch.Size([]), resample: bool = True) -> torch.Tensor:
        z = self.inducing_points # [M, D]

        # Step 1. Get prior samples from RFF
        Phi_w_x, Phi_w_z = self.sample_prior(x=x, z=z, sample_shape=sample_shape, resample=resample) # [S, O, N], [S, O, M] or [S, N], [S, M]

        # Step 2. Get prior sample from VI
        u = self.sample_variational(sample_shape=sample_shape, resample=resample) # [S, O, M] or [S, M]

        # Step 3. Update prior sample with VI sample 
        return Phi_w_x + self.compute_posterior_update(x=x, z=z, u=u, Phi_w_z=Phi_w_z, sample_shape=sample_shape) # [S, O, N] or [S, N]
