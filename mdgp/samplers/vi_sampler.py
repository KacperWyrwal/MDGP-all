...
class VISampler(torch.nn.Module):
    ...
    def base_samples(self, sample_shape: torch.Size, resample=True):
    ...
    def forward(self, sample_shape: torch.Size = torch.Size([]), resample: bool = True):
        return self.variational_distribution().rsample(sample_shape=sample_shape, base_samples=self.base_samples(sample_shape, resample=resample))
